...
def image_level_metrics(results, obj, metric):
    ...
    elif metric == 'image-f1max':
        precisions, recalls, _ = precision_recall_curve(gt.ravel(), pr.ravel())
...
def pixel_level_metrics(results, obj, metric):
    ...
    '''
    if metric == 'aupimo':
        pimore, aupimo = pimo.aupimo_scores(
            pr,
            gt,
            num_threshs=gt.shape[0],
            binclf_algorithm="numba",
            shared_fpr_metric="mean-per-image-fpr",
            force=True,
        )
        isnan = torch.isnan(aupimo.aupimos)
        aupimo_avg = aupimo.aupimos[~isnan].mean().item()
    return aupimo_avg
    '''
    gt = np.array(gt)
    pr = np.array(pr)
