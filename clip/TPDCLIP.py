...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, need_weights: bool = False):
        ...
        self.resblocks = nn.Sequential(
            *[ResidualAttentionBlock(width, heads, attn_mask, need_weights if i == layers - 1 else False) for i in
              range(layers)])
...
class VisionTransformer(nn.Module):
    ...
    def VVattn_replace(self, VVattn_layers):
        # reform the architecture during first inference
        if VVattn_layers is not None:
            # apply architecture surgery on the last VVattn_layers blocks
            for i in range(1, VVattn_layers):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x[0, :, :] = x_ori[0, :, :]  # keep original cls token
